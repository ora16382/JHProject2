   ...
   <script type="text/javascript" >
	...
	//수정 반영하는것
	 function fn_modify_article(obj){
		 obj.action="${contextPath}/hboard/modArticle.do";
		 obj.submit();
	 }
	 ...
	 function add_comment(articleNO, obj){
		 if($(obj).prev().val()!=""){
		 $.ajax({
			url:"${contextPath}/hboard/addNewComment",
			type:"post",	 
...
<h6 style="border: 1px solid grey; width: 75px; margin-bottom:10px; margin-left: 28em; padding: 1px; " align="right">조회수 :  ${article.views}</h6>
  <form name="frmArticle" method="post"  action="${contextPath}/hboard/modArticle.do"  enctype="multipart/form-data">
...
<td><div id="d_file">  
	...
	<img  id='preview${status.count}' src='${contextPath}/download.do?articleNO=${article.articleNO}&imageFileName=${item.imageFileName}&directory=humor'   width=150 height=150 onClick="viewImage(this)"/><br>
	...
	<script>
		...
		imageList[${status.index}] = new Image();
   		imageList[${status.index}].src='${contextPath}/download.do?articleNO=${article.articleNO}&imageFileName=${item.imageFileName}&directory=humor';
	</script>
	</c:forEach>
	</div></td>
	</tr>  
 ...
 <c:otherwise>
  ...
  <tr  id="tr_btn"    >
   ...
   <c:if test="${member.id==article.id || member.id=='wnsgud14' }">
	    ...
	    <input type=button value="삭제하기" onClick="fn_remove_article('${contextPath}/hboard/removeArticle.do', ${article.articleNO},'${article.id}')">
	</c:if>
	    <input type=button value="리스트로 돌아가기"  onClick="backToList(this.form)">
	     <input type=button value="답글쓰기"  onClick="fn_reply_form('${contextPath}/hboard/replyForm.do', ${article.articleNO},'${article.title }')">
   </td>
  </tr>
 </table>
 </form>
