...
<script>
 	function fn_articleForm(isLogOn,id,articleForm,loginForm){
 		if(isLogOn !=''&&id!=''){
 			...
 			location.href=loginForm+'?action=/gboard/articleForm.do';
 		}
 	}
 </script>
	   ...
	   <c:choose>
	      ...
	      <c:when test='${article.level > 1 }'>  
	         ...
	         <span style="font-size:12px;">[답변]</span>
                   <a class='cls1' href="${contextPath}/gboard/viewArticle.do?articleNO=${article.articleNO}">${article.title}</a>
	          </c:when>
      ...
      <%-- 글 개수가 100 초과인경우 --%>
        <c:when test="${totArticles >=100 }"> 
	      ...
	      <%--2섹션 이상일때 이전 섹션으로 가는 텍스트 출력 --%>
	         <c:if test="${section >1 && page==1 }">
	          <a class="no-uline" href="${contextPath }/gboard/listArticles.do?section=${section-1}&pageNum=10">&nbsp; Pre </a>
	         </c:if>
	         
	         
	          <c:choose>
	           <c:when test="${page==pageNum }">
	          <a class="sel-page" style="color:aqua" href="${contextPath }/gboard/listArticles.do?section=${section}&pageNum=${page}">${(section-1)*10 +page } </a>
	          </c:when>
	          <c:otherwise>
	            <a class="no-uline"  href="${contextPath }/gboard/listArticles.do?section=${section}&pageNum=${page}">${(section-1)*10 +page }  </a>
	          </c:otherwise>
	         </c:choose>
	         ...
	         <c:if test="${page ==10 }">
	         	<c:if test="${totArticles >100 }">
	          	<a class="no-uline" href="${contextPath }/gboard/listArticles.do?section=${section+1}&pageNum=1">&nbsp; Next</a>
	         	 </c:if>
	         </c:if>
	        </c:forEach>
	       </c:when>
        ...
        <c:when test="${totArticles< 100 }" > 
	      <c:forEach   var="page" begin="1" end="${totArticles/20 +1}" step="1" >
	         <c:choose>
	           <c:when test="${page==pageNum }">
	            <a class="sel-page" style="color:aqua" href="${contextPath }/gboard/listArticles.do?section=${section}&pageNum=${page}">${page } </a>
	          </c:when>
	          <c:otherwise>
	            <a class="no-uline"  href="${contextPath }/gboard/listArticles.do?section=${section}&pageNum=${page}">${page } </a>
	          </c:otherwise>
	        </c:choose>
	      </c:forEach>
        </c:when>
...
<br>
<a href="javascript:fn_articleForm('${isLogOn}','${member.id }','${contextPath }/gboard/articleForm.do','${contextPath }/member/loginForm.do')"><span class="cls3">글쓰기</span></a>
</body>
</html>