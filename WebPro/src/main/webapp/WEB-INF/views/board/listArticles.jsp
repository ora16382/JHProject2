      ...
      <%-- 글 개수가 100 초과인경우 --%>
        <c:when test="${totArticles >=100 }"> 
	      ...
	      <%--2섹션 이상일때 이전 섹션으로 가는 텍스트 출력 --%>
	         <c:if test="${section >1 && page==1 }">
	          ...
	          <a class="sel-page" style="color:aqua" href="${contextPath }/board/listArticles.do?section=${section}&pageNum=${page}">${(section-1)*10 +page } </a>
	          </c:when>
	          <c:otherwise>
        ...
        <c:when test="${totArticles< 100 }" > 
	      <c:forEach   var="page" begin="1" end="${totArticles/20 +1}" step="1" >
	         <c:choose>
	           <c:when test="${page==pageNum }">
	            <a class="sel-page" style="color:aqua" href="${contextPath }/board/listArticles.do?section=${section}&pageNum=${page}">${page } </a>
	          </c:when>
	          <c:otherwise>
...
<br>
<a href="javascript:fn_articleForm('${isLogOn}','${member.id }','${contextPath }/board/articleForm.do','${contextPath }/member/loginForm.do')"><span class="cls3">글쓰기</span></a>
</body>
</html>