   ...
   <script type="text/javascript" >
	...
	//수정 반영하는것
	 ...
	 function add_comment(articleNO, obj){
		 $.ajax({
			url:"${contextPath}/board/addNewComment",
			type:"post",	 
...
<td><div id="d_file">  
	...
	<img  id='preview${status.count}' src='${contextPath}/download.do?articleNO=${article.articleNO}&imageFileName=${item.imageFileName}&directory=free'   width=150 height=150 onClick="viewImage(this)"/><br>
	...
	<script>
		...
		imageList[${status.index}] = new Image();
   		imageList[${status.index}].src='${contextPath}/download.do?articleNO=${article.articleNO}&imageFileName=${item.imageFileName}&directory=free';
	</script>
	</c:forEach>
	</div></td>
	</tr>  
 ...
 <c:otherwise>
  ...
  <tr  id="tr_btn"    >
   ...
   <c:if test="${member.id==article.id || member.id=='wnsgud14' }">
	    ...
	    <input type=button value="삭제하기" onClick="fn_remove_article('${contextPath}/board/removeArticle.do', ${article.articleNO},'${article.id}')">
	</c:if>
 ...
 <table border="0" align="center" id="commentTable">
	<c:forEach var="com" items="${commentList}" varStatus="status">
		...
		<c:otherwise>
		<tr><td class='writer'>작성자 : ${com.id}</td><td class='comment'>${com.body}</td></tr>
		</c:otherwise>
		</c:choose>
